...
.PARAMETER Down
  Stop containers: docker compose down (keeps volumes).

.PARAMETER NoBuild
  Skip image build; only docker compose up -d.

.PARAMETER Pull
  Run docker compose pull before build.

.PARAMETER Help
  Show this help.

.PARAMETER AiValidator
  After FleetShare is up: clone (if needed) and start the Gemini driving-licence service from
  https://github.com/dan123-tech/AI_driving-licence (folder next to this project). Requires git.
...
if (-not (Test-Path (Join-Path $Root "docker-compose.yml"))) {
  Write-Err "docker-compose.yml not found. Run this script from the FleetShare project root."
  exit 1
}
...
# AUTH_SECRET length hint (line-based, no dot-sourcing)
...
if ($AiValidator) {
  ...
  Write-Host ""
  Write-Host "Validator: http://localhost:8080/health  (FleetShare uses host.docker.internal:8080)" -ForegroundColor Green
  Write-Host "Stop validator: cd `"$AiDir`"; docker compose down" -ForegroundColor DarkGray
  Write-Host ""
}
