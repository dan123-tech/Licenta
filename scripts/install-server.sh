#!/usr/bin/env bash
# FleetShare — install / update / stop via Docker Compose (Linux / macOS).
# Run from repo root: ./install.sh   or   bash scripts/install-server.sh [options]
#
# Options:
#   --help, -h       Show usage
#   --down           Stop containers (docker compose down)
#   --no-build       Start without rebuilding images (faster if code unchanged)
#   --pull           Pull base images before build
#   --ai-validator   After FleetShare is up: clone/start AI_driving-licence (Gemini) beside this project
set -euo pipefail
...
Usage: ./install.sh  |  bash scripts/install-server.sh  [options]
...
Options:
  ...
  --pull              Pull base images before build
  --ai-validator      Clone/start AI driving-licence validator (Gemini) beside this project; needs git + GEMINI_API_KEY in validator .env
...
# Light check: AUTH_SECRET length (do not source .env — may contain special chars)
...
echo "  • Stop:  ./install.sh --down   or   bash scripts/install-server.sh --down"
echo "  • Driving licence AI (optional): repo ${AI_VALIDATOR_REPO%.git} — run with --ai-validator or see /download"
echo ""
...
if [[ "$DO_AI_VALIDATOR" == true ]]; then
  ...
  if ! command -v git >/dev/null 2>&1; then
    echo -e "${RED}git not found. Install git, then clone:${RST}"
    echo "  git clone $AI_VALIDATOR_REPO \"$AI_DIR\""
    echo "  cd \"$AI_DIR\" && cp .env.example .env  # set GEMINI_API_KEY"
  ...
  if [[ ! -f "$AI_DIR/.env" ]]; then
    if [[ -f "$AI_DIR/.env.example" ]]; then
      ...
      echo -e "${YLW}==>${RST} Created ${GRN}$AI_DIR/.env${RST} from .env.example — add ${YLW}GEMINI_API_KEY${RST} before validation will work."
    else
      echo -e "${YLW}⚠ No .env in validator repo. Create $AI_DIR/.env with GEMINI_API_KEY (see repo README).${RST}"
    fi
  fi
  echo -e "${CYA}==>${RST} Building & starting validator (port 8080)…"
  ( cd "$AI_DIR" && $COMPOSE_CMD up -d --build )
  echo ""
  echo -e "${GRN}Validator:${RST} ${CYA}http://localhost:8080/health${RST}  ·  FleetShare app uses ${DIM}host.docker.internal:8080${RST} (see docker-compose.yml)"
  echo -e "${DIM}Stop validator separately: cd \"$AI_DIR\" && $COMPOSE_CMD down${RST}"
  echo ""
fi
